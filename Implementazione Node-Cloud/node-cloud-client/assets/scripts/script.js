...
const colorArray = [
    "#a4e43f", "#d298e2", "#6119d0", "#d2737d", "#c0a43c", "#f2510e", "#651be6",
    "#79806e", "#61da5e", "#cd2f00", "#9348af", "#01ac53", "#c5a4fb", "#996635",
    "#b11573", "#4bb473", "#75d89e", "#63b598", "#ce7d78", "#ea9e70", "#a48a9e",
    ...
    "#c6e1e8", "#648177", "#0d5ac1", "#f205e6", "#1c0365", "#14a9ad", "#4ca2f9",
]
...
async function sampleReceived(sample) {
    ...
    if (sensor.samples && sensor.samples.length == 1 && (box != null || box != undefined)) {
        swapToSensor(sensor);
    } else {
    ...
    sensorFormVm = new Vue({
        ...
        methods: {
            ...
            sampleTimeChanged(index) {
                ...
                let enabled = false;
                if (sampleTime < 1000) {
                    sensor.isNotValid = 'Invalid sample time. Min = 1000. Up arrow to fix';
                } else if (sampleTime > 86400000) {
                    sensor.isNotValid = 'Invalid sample time. Max = 86400000. Down arrow to fix.';
                } else if ((sampleTime % 1000) != 0) {
                    sensor.isNotValid = 'Invalid sample time. Step = 1000. Any arrow to fix.';
                } else {
                    sensor.isNotValid = '';
                    enabled = true;
                }
            ...
            changeSensor(index) {
                if (this.sampleTimeChanged(index)) {
                    ...
                    sensor.precision = "+/-" + formSensor.precision + evaluateContext(sensor.context);
                    updateSensor(sensor);
                    window.alert("Sensor: " + sensor.code + " has been updated!");
...
function swapToManageNodes() {
    ...
    for (var i = 0; i < nodesPayload.length; i++) {
        ...
        delete node.sensors;
        node.disableButton = true;
        if (!nodeFormVm.contains(node.nodeId))
            nodeFormVm.addNodeForm(node)
    }
...
function generateId() {
    var date = new Date();
    return date.getFullYear() + ((date.getMonth() + 1) < 9 ? "0" + (date.getMonth() + 1) : (date.getMonth() + 1)) + (date.getDate() < 9 ? "0" + date.getDate() : date.getDate()) + (date.getHours() < 9 ? "0" + date.getHours() : date.getHours()) + (date.getMinutes() < 9 ? "0" + date.getMinutes() : date.getMinutes() + (date.getSeconds() < 9 ? "0" + date.getSeconds() : date.getSeconds()));
}
...
function removeNodeScrollBar() {
    ...
    for (let i = 0; i < nodesPayload.length; i++) {
        if (document.getElementById(nodesPayload[i].nodeId + 'li'))
            ul.removeChild(document.getElementById(nodesPayload[i].nodeId + 'li'));
    }
...
function createNodeElement(node) {
    ...
    if(node.name)
        ...
        nodeName.innerHTML = "Unknown";
    var link = document.createElement('a');
    link.href = "#";
    var i1 = document.createElement('i');
    var i2 = document.createElement('i');
    i1.className = node.isAvailable ? "metismenu-icon pe-7s-check" : "metismenu-icon pe-7s-close-circle";
...
function createSensorElement(sensor, listElement) {
    ...
    if(sensor.code)
        sensorCode.innerHTML = sensor.code;
    else
        sensorCode.innerHTML = "Unknown";
...
function swapToSensor(sensor) {
    if(sensor){
        ...
        if(sensor.code)
            createSensorCodeBox(sensor.code, row1);
        
        if(sensor.context)    
            createContextBox(sensor.context, row1);
        
        
        createAvailableBox(sensor, row1);

        if(sensor.sampleTime)
            createSampleTimeBox(sensor, row1);
        ...
        if(sensor.precision)
            createPrecisionBox(sensor.precision, row1);
...
function createPropertyBox(propertyName, property, element) {
    ...
    var widgetDivHeading = document.createElement('div');
    widgetDivHeading.className = "widget-heading";
    widgetDivHeading.innerText = propertyName.charAt(0).toUpperCase() + propertyName.slice(1);
...
function updateLastSampleBox(sensor) {
    ...
    var sample = findLastSample(sensor.samples);
    var box = document.getElementById(id);
    if ((box != null || box != undefined) && (sample != null || sample != undefined)) {
        var value = Math.round(sample.value * 100) / 100;
        value += evaluateContext(sensor.context);
        box.innerText = value;
    }
...
function createTotalSamplesBox(sensor, element) {
    ...
    var widgetDivValue = document.createElement('div');
    widgetDivValue.className = "widget-numbers text-primary";
    widgetDivValue.innerText = sensor.samples != null ? sensor.samples.length : 0;
...
function createSamplesAverageBox(sensor, element) {
    ...
    var widgetDivValue = document.createElement('div');
    widgetDivValue.className = "widget-numbers text-warning";
    var average = sensor.samples != null ? calculateSamplesAverage(sensor.samples, 0.125) : 0;
...
function updateAvailableSensors() {
    var availableSensorsSpan = document.getElementById('available-sensors-span');
...
function getTotalSamplesFromSensors() {
    ...
    for (var i = 0; i < nodesPayload.length; i++) {
        if (nodesPayload[i].sensors) {
            for (var j = 0; j < nodesPayload[i].sensors.length; j++) {
                totalSamples.push(nodesPayload[i].sensors[j].samples != null ? nodesPayload[i].sensors[j].samples.length : 0);
            }
        }
    }
    return totalSamples;
...
function getTotalSamplesFromNodes() {
    ...
    for (var i = 0; i < nodesPayload.length; i++) {
        if (nodesPayload[i].sensors) {
            for (var j = 0; j < nodesPayload[i].sensors.length; j++) {
                sum += nodesPayload[i].sensors[j].samples != null ? nodesPayload[i].sensors[j].samples.length : 0;
            }
